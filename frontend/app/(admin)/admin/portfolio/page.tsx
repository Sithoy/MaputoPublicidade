...
export default function AdminPortfolioPage() {
  ...
  async function handleDelete() {
    ...
    try {
      await deletePortfolioItem(deleteSlug);
      await loadItems();
      setDeleteSlug(null);
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-dark">Portfólio</h1>
          <p className="text-sm text-gray-500">Gerir trabalhos e projectos</p>
        </div>
      ...
      <DataTable
        columns={[
          ...
          { key: 'client_name', header: 'Cliente' },
          {
            key: 'completion_date',
            header: 'Data',
            render: (item) => item.completion_date || '-',
          },
          {
            key: 'is_active',
            header: 'Activo',
        ...
        actions={(item) => (
          <div className="flex items-center gap-2">
            <Link href={`/admin/portfolio/${item.slug}/editar`}>
      ...
      <ConfirmDialog
        open={!!deleteSlug}
        title="Eliminar item do portfólio"
        message="Tem a certeza que deseja eliminar este item? Esta acção não pode ser desfeita."
